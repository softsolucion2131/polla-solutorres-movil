...
function AdminResultadosPage() {
  ...
  // Limpiar selecciones dependientes si cambia la fecha
  useEffect(() => {
    setSelectedHip("");
    setSelectedCarrera("");
  }, [fecha]);
  ...
  const mutationProcesar = useMutation({
    mutationFn: async () => {
      ...
      const numCarrera = Number(selectedCarrera); // Casteo explícito a Number
    ...
    onSuccess: () => {
      ...
      // Revalidación de caché completa
      qc.invalidateQueries({ queryKey: ["admin-carreras", selectedHip, fecha] });
      qc.invalidateQueries({ queryKey: ["pollas-stats"] });
      qc.invalidateQueries({ queryKey: ["pollas-publicas", selectedHip, fecha] });
      qc.invalidateQueries({ queryKey: ["pollas"] });
    },
    onError: (err: any) => {
